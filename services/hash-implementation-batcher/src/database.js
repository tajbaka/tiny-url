...
class UrlDatabase {
  ...
  // Get multiple long URLs by short URLs (batch processing)
  static async getLongUrlsBatch(shortUrls) {
    try {
      if (!shortUrls || shortUrls.length === 0) {
        return {};
      }

      // Use ANY() to match against array of short URLs
      const result = await urlsPool.query(
        "SELECT short_url, long_url FROM urls WHERE short_url = ANY($1)",
        [shortUrls]
      );

      // Convert result to a map for easy lookup
      const urlMap = {};
      ...
      console.log(
        `[DB] Batch lookup: requested ${shortUrls.length} URLs, found ${result.rows.length} matches`
      );

      return urlMap;
    } catch (err) {
      console.error("Error getting long URLs in batch:", err);
      throw err;
    }
  }
