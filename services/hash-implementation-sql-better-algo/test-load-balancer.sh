...
test_nginx_status() {
    ...
    curl -s http://localhost:8100/nginx-status
    echo ""
}
...
test_instance_distribution() {
    ...
    for i in {1..10}; do
        instance_id=$(curl -s http://localhost:8100/api/instance | jq -r '.instanceId')
...
test_url_shortening() {
    ...
    for i in {1..5}; do
        ...
        response=$(curl -s -X POST http://localhost:8100/api/shorten \
...
test_memory_usage() {
    ...
    for i in {1..3}; do
        ...
        curl -s http://localhost:8100/memory | jq '.'
        echo ""
        sleep 0.1
    done
}
