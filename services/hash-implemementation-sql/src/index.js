...
app.get("/api/stats/:shortUrl", async (req, res) => {
  try {
    ...
    const stats = await UrlDatabase.getUrlStats(shortUrl);
    ...
    console.error(`[Instance ${instanceId}] Error in /api/stats:`, error);
    res.status(500).json({ error: "Failed to get URL statistics" });
  }
});

// Admin endpoint to get recent URLs
app.get("/api/admin/recent", async (req, res) => {
  try {
    const limit = parseInt(req.query.limit) || 10;
    const recentUrls = await UrlDatabase.getRecentUrls(limit);
    ...
    console.error(
      `[Instance ${instanceId}] Error in /api/admin/recent:`,
      error
    );
...
app.get("/:shortUrl", async (req, res) => {
  try {
    const { shortUrl } = req.params;
    console.log(`[Instance ${instanceId}] Redirect request for: ${shortUrl}`);

    const longUrl = await UrlDatabase.getLongUrl(shortUrl);
    ...
    UrlDatabase.incrementClickCount(shortUrl).catch((err) => {
      console.error(
