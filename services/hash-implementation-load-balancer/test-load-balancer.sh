...
echo "🔥 Testing Hash LB Load Balancer with Round Robin Distribution"
echo "====================================================="
...
test_nginx_status() {
    ...
    curl -s http://localhost:8090/nginx-status
    echo ""
}
...
test_instance_distribution() {
    ...
    for i in {1..10}; do
        instance_id=$(curl -s http://localhost:8090/api/instance | jq -r '.instanceId')
...
test_url_shortening() {
    ...
    for i in {1..5}; do
        ...
        response=$(curl -s -X POST http://localhost:8090/api/shorten \
...
test_memory_usage() {
    ...
    for i in {1..3}; do
        ...
        curl -s http://localhost:8090/memory | jq '.'
        echo ""
        sleep 0.1
    done
}
...
test_container_health() {
    ...
    docker ps --filter "name=hash-lb-service" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
    echo ""
    docker ps --filter "name=hash-lb-nginx" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
    echo ""
}

# Main execution
echo "Starting Hash LB Load Balancer Tests..."
...
echo "✅ Hash LB Load Balancer Test Complete!"
echo ""
echo "📊 To monitor logs in real-time:"
echo "   docker-compose logs -f"
echo ""
echo "🔍 To see nginx access logs:"
echo "   docker-compose logs hash-lb-nginx" 