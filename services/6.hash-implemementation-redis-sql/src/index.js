...
app.use((req, res, next) => {
  ...
  res.setHeader("X-Service-Name", "postgres-redis-service");
  next();
});
...
app.get("/api/instance", async (req, res) => {
  try {
    ...
    res.status(200).json({
      instanceId: instanceId,
      serviceName: "postgres-redis-service",
      database: "PostgreSQL",
      cache: "Redis LFU",
      uptime: process.uptime(),
...
// Add this endpoint after /api/urls and before the catch-all route
app.get("/api/mappings", async (req, res) => {
  try {
    // Optionally support a limit query param
    const limit = parseInt(req.query.limit) || 1000;
    const mappings = await UrlDatabase.getUrls(limit);
    res.json({
      mappings,
    ...
    console.error(`[Instance ${instanceId}] Error in /api/mappings:`, error);
    res.status(500).json({ error: "Failed to fetch URL mappings", instanceId });
  }
});
